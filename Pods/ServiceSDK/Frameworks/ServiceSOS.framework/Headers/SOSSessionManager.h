 ...
 *                __NOTE__: Error is `nil` if the session ended cleanly.
 */
 ...
 *
 *  @param options `<SOSOptions>` object that determines the behavior of this session.
 *  @param block Completion block which is executed when the session has been fully connected to all services.<br/>
 *                __NOTE:__ At this point the session is active and waiting for an agent to join.<br/>
 *                __NOTE:__ The NSError returned in the block is `nil` on success.
 */
 ...
 *
 *  #### Delegate Methods:
 *  - `<[SOSDelegate sos:didStopWithReason:error:]>`<br/>
 ...
 *    Invoked if there is an error at any point during the lifetime of a session.
 *
 *  <br/>__NOTE:__ The NSError returned in the block is `nil` on success.
 *
 *  @see SOSDelegate
 *
